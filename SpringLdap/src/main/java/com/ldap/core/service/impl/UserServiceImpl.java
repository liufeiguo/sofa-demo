...
public class UserServiceImpl implements UserService {
    ...
    public boolean isValid(String uid, String password) {
        try {
            ...
            //filter.and(new EqualsFilter("ou", "deptment"));
            //filter.and(new EqualsFilter("ou", "EADept"));
            filter.and(new EqualsFilter("uid", uid));
            
            String dnByUid = getDnByUid(uid);
           // uid=liu1,ou=EADept,ou=deptment,ou=utrust,dc=chinautrust,dc=com
            boolean valid = ldapTemplate.authenticate("", filter.toString(), password);
    ...
    public boolean updatePwd(String uid, String passpord) {
        try {
            ...
            String dnByUid = getDnByUid(uid);
            ldapTemplate.modifyAttributes(dnByUid, item);
    ...
    public String getDnByUid(String uid) {
        return getUserDn("uid", uid).replace(",ou=EADept,ou=deptment,ou=utrust,dc=chinautrust,dc=com", "");
    }
